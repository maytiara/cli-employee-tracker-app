...
async function addEmployee (first_name, last_name, role_id, manager_id){
  ...
  const query = await `INSERT INTO employees (first_name, last_name, role_id, manager_id) VALUES (?,?,?,?);`;
...
async function getEmployees (){
  ...
  const [employees] = await db.query ('SELECT employees.id AS `ID`, employees.first_name AS `FIRST NAME`, employees.last_name AS `SURNAME`, roles.title AS `TITLE`, manager.first_name AS `MANAGER` FROM employees LEFT JOIN roles ON employees.role_id = roles.id JOIN employees AS manager WHERE employees.manager_id = manager.id'); // in JOIN (we deceived the MySQL by passing a dummy field name and get the manager(value) from the same table
